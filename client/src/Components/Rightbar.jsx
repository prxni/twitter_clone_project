                    ...
                    {following.length!=0 &&
                        <div>
                            <p className="text-md text-rose-400 font-semibold">Following</p>  
                                {following.map(user => {
                                    return <p key={user.id} className="px-6 py-2 flex gap-2 cursor-default item-center hover:bg-gray-100 rounded-md items-center group"><span className="material-symbols-outlined">person</span>{user.username}<span className="text-gray-400 text-xs group-hover:opacity-100 opacity-0 duration-150">({user.name})</span></p>
                                })}
